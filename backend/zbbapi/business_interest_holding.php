...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("
        INSERT INTO business_interest_holdings (
            ...
            intended_successor, successor_is_family, should_business_be_sold, ownership_form, notes, owned_by, other_owners, has_bene
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param(
        "issddississsssssssss",
        $data['portal_user_id'],
        $data['business_name'],
        $data['business_type'],
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !isset($_GET['id'])) {
    $result = $conn->query("SELECT * FROM business_interest_holdings");
    $rows = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT * FROM business_interest_holdings WHERE business_interest_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("DELETE FROM business_interest_holdings WHERE business_interest_id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
