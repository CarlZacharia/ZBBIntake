...
export class AuthService {
        ...
        return this.http.post<IAuthResponse>(`${this.API_URL}/login`, credentials).pipe(
            tap(response => {
                if (response.success && response.user && response.token) {
                    this.setAuthData(response.user, response.token, response.refresh_token, response.expires_in);
                }
            }),
    ...
    register(registerData: IRegisterData): Observable<IAuthResponse> {
        ...
        return this.http.post<IAuthResponse>(`${this.API_URL}/register`, registerData).pipe(
            tap(response => {
                if (response.success && response.user && response.token) {
                    this.setAuthData(response.user, response.token, response.refresh_token, response.expires_in);
                }
